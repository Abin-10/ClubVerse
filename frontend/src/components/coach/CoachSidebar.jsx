...
export default function CoachSidebar({ activeTab, setActiveTab }) {
  const menuItems = [
    { id: 'overview', icon: Home, label: 'Overview' },
    { id: 'players', icon: Users, label: 'Player Management' },
    { id: 'training', icon: Calendar, label: 'Training Management' },
    { id: 'performance', icon: BarChart2, label: 'Performance Analysis' },
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-16 sm:w-20 h-screen bg-[#FFFDF8] border-r border-[#E4E1D8] flex flex-col items-center py-6 justify-between flex-shrink-0 z-50 shadow-warm-sm overflow-y-auto font-sans">
      {/* Brand Icon / Coach Badge */}
      <div className="flex flex-col items-center gap-6">
        <motion.div 
          whileHover={{ scale: 1.1, rotate: 5 }}
          whileTap={{ scale: 0.95 }}
          onClick={() => setActiveTab('overview')}
          title="ClubVerse Coach Portal"
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.span 
                    layoutId="activeCoachSidebarIndicator"
